...
#if JS_HAS_XML_SUPPORT
    // haleyjd: change from https://bugzilla.mozilla.org/show_bug.cgi?id=514309
    // "Object.prototype.hasOwnProperty mishandles non-primitive property name"
    if (!JSVAL_IS_PRIMITIVE(v)) {
        JSClass *clasp = OBJ_GET_CLASS(cx, JSVAL_TO_OBJECT(v));
...
// haleyjd 20150507: Internals of JS_Enumerate, separated for addition of a
// JS_EnumerateAll API call.
static JSIdArray *EnumerateInternal(JSContext *cx, JSObject *obj, JSIterateOp op)
{
    jsint i, n;
    ...
    if (!OBJ_ENUMERATE(cx, obj, op, &iter_state, &num_properties))
        goto error;
...
{
    return EnumerateInternal(cx, obj, JSENUMERATE_INIT);
}

// haleyjd 20150507: Custom API call, added to cover for shortcomings with
// inability to otherwise visit non-enumerable object properties through the
// JSAPI.
JS_PUBLIC_API(JSIdArray *)
JS_EnumerateAll(JSContext *cx, JSObject *obj)
{
    return EnumerateInternal(cx, obj, JSENUMERATE_INIT_ALL);
}
