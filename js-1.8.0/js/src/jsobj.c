             ...
             jsval *statep, jsid *idp)
{
    ...
    switch (enum_op) {
      ...
      case JSENUMERATE_INIT_ALL: // haleyjd 20150507: adapted from JSAPI 1.8.5
        if (!enumerate(cx, obj))
            return JS_FALSE;
        length = 0;
        ...
        if (proto && scope == OBJ_SCOPE(proto)) {
            ...
            for (sprop = lastProp = SCOPE_LAST_PROP(scope); sprop;
                 sprop = sprop->parent) {
                if (((sprop->attrs & JSPROP_ENUMERATE) || enum_op == JSENUMERATE_INIT_ALL) && // haleyjd 20150507: add INIT_ALL
